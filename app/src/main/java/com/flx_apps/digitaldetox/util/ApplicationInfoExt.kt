 ...
 * title from the package name (for known apps, hard-coded above). If that fails, it obtains the
 * title from the category, if available by the system (see [ApplicationInfo.category]).
...
fun ApplicationInfo.getAppCategoryTitle(
    ...
    var stringResourceId: Int = when (this.packageName) {
